...
const Dashboard = () => {
  const { totalScans, violationsBlocked, activeThreatAgents, systemUptime, threatLevel, trafficVelocity, intentDistribution, liveAuditTrail } = dashboardData;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        ...
        <motion.div 
          ...
          animate={threatLevel === 'CRITICAL' || threatLevel === 'HIGH' ? { scale: [1, 1.02, 1], boxShadow: `0 0 20px ${threatColor}40` } : {}}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.5rem' }}>
        <div className="card" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '1.8rem', fontWeight: 700 }}><CountUp end={totalScans} duration={2} /></div>
          </div>
        </div>
        <div className="card" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '1.8rem', fontWeight: 700 }}><CountUp end={violationsBlocked} duration={2} /></div>
          </div>
        </div>
        <div className="card" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '1.8rem', fontWeight: 700 }}><CountUp end={activeThreatAgents} duration={2} /></div>
          </div>
        </div>
        <div className="card" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '1.8rem', fontWeight: 700 }}>{systemUptime}%</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1.5rem', height: '350px' }}>
        <div className="card" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={trafficVelocity}>
        ...
        <div className="card" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={intentDistribution} innerRadius={60} outerRadius={90} paddingAngle={5} dataKey="value" stroke="none">
                {intentDistribution.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
              </Pie>
      ...
      <div className="card" style={{ flex: 1 }}>
        ...
        <div className="table-responsive">
          <table>
            ...
            <tbody>
              {liveAuditTrail.map(log => (
                <motion.tr key={log.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} 
                           style={{ background: log.violation ? 'rgba(255,51,102,0.05)' : 'transparent' }}>
                  ...
                  <td>
                    ...
                    <div style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>{log.ip}</div>
                  </td>
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '4px' }}>
                      <span className={`badge badge-${log.intent}`}>{log.intent}</span>
                    </div>
                    {log.type && <div style={{ color: 'var(--danger)', fontSize: '0.8rem', fontWeight: 'bold' }}>{log.type}</div>}
                  </td>
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <div className="progress-container" style={{ width: '60px' }}>
                         <div className="progress-bar" style={{ width: `${(log.confidence) * 100}%`, background: log.confidence < 0.70 ? 'var(--warning)' : 'var(--success)' }}></div>
                      </div>
                      <span style={{ fontSize: '0.8rem' }}>{Math.round(log.confidence*100)}%</span>
                    </div>
                  </td>
                  <td>
                    <div className="progress-container" style={{ width: '60px', background: 'rgba(255,51,102,0.1)' }}>
                       <div className="progress-bar" style={{ width: `${Math.abs(log.anomaly * 100)}%`, background: 'var(--danger)' }}></div>
                    </div>
                  </td>
                </motion.tr>
              ))}
